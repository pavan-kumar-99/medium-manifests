...
def process_data(local_path):
    ...
    df = spark.read.option("multiLine", "true").option("mode", "PERMISSIVE").schema(articlesSchema).json(local_path)
    df_posts = spark.read.option("multiLine", "true").option("mode", "PERMISSIVE").json(file_path)

    # Data transformations
    df = df.dropDuplicates()  # Drop duplicate rows
    df_posts = df_posts.dropDuplicates()
    df = df.withColumn('Date', to_date(from_unixtime(floor(col('dayStartsAt') / 1000), 'yyyy-MM-dd HH:mm:ss')))
...
if __name__ == "__main__":
    ...
    parser.add_argument('local_path', type=str, help='Local path where the file will be downloaded')
    parser.add_argument('ingest_mode', type=str, choices=['append', 'create'], help='Ingestion mode: append or create')
